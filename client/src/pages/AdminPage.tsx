...
function ProjectEditor({ projects, setProjects, onPersistProjects }: { projects: ProjectDraft[]; setProjects: React.Dispatch<React.SetStateAction<ProjectDraft[]>>; onPersistProjects: (projects: ProjectDraft[], message: string) => Promise<void> }) {
  ...
  const editProject = (index: number) => {
    ...
    toast.success("Proyecto listo para editar", { description: "Actualizá los datos, guardá el borrador y revisá la vista previa." });
  };
  const deleteProject = (index: number) => {
    const project = projects[index];
    if (!project || !window.confirm(`¿Eliminar “${project.title}” del borrador? El sitio público no cambia hasta que publiques los cambios.`)) return;
        ...
        <article id={`project-card-${index}`} tabIndex={-1} key={project.id ?? `new-${index}`} className="scroll-mt-24 overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
          <div className="grid gap-0 lg:grid-cols-[280px_1fr]">
            ...
            <div className="space-y-4 p-5">
              <div className="flex flex-wrap items-center justify-between gap-3">
                <span className="text-xs font-bold uppercase tracking-[0.14em] text-slate-500">Proyecto {String(index + 1).padStart(2, "0")}</span>
                <div className="flex flex-wrap items-center gap-2">
                  <Button type="button" variant="outline" size="sm" onClick={() => editProject(index)}><Pencil className="mr-2 h-4 w-4" />Editar proyecto</Button>
                  <label className="flex items-center gap-2 text-sm font-medium"><input type="checkbox" checked={project.visible} onChange={event => updateProject(index, { visible: event.target.checked })} /> Visible al publicar</label>
                </div>
              </div>
              <div className={`rounded-lg border px-3 py-2 text-sm leading-5 ${imageMessages[index]?.startsWith("No se pudo") || imageMessages[index]?.startsWith("Usá") ? "border-red-200 bg-red-50 text-red-700" : imageMessages[index] ? "border-emerald-200 bg-emerald-50 text-emerald-800" : "border-slate-200 bg-slate-50 text-slate-600"}`}>{imageMessages[index] || "Paso 1: editá los datos o subí una foto. Paso 2: guardá el borrador. Paso 3: revisá y publicá."}</div>
              <div className="grid gap-3 md:grid-cols-2">
                <Input aria-label="Categoría" value={project.category} onChange={event => updateProject(index, { category: event.target.value })} placeholder="Categoría" />
                <Input aria-label="Título" value={project.title} onChange={event => updateProject(index, { title: event.target.value })} placeholder="Título" />
              </div>
              ...
              <div className="flex flex-wrap items-center justify-between gap-2">
                <div className="flex gap-2">
                  <Button type="button" size="icon" variant="outline" onClick={() => move(index, -1)} disabled={index === 0} aria-label="Mover arriba"><ArrowUp className="h-4 w-4" /></Button>
                  <Button type="button" size="icon" variant="outline" onClick={() => move(index, 1)} disabled={index === projects.length - 1} aria-label="Mover abajo"><ArrowDown className="h-4 w-4" /></Button>
                </div>
                <Button type="button" variant="destructive" size="sm" onClick={() => deleteProject(index)}><Trash2 className="mr-2 h-4 w-4" />Eliminar proyecto</Button>
              </div>
            </div>
          </div>
        </article>
